...
// every stroke automatically. The magnifying-glass disc (formerly a
// hardcoded `#fff`) is parameterized so the dark theme can swap it from
// "paper" to "ink" — matching how the header brand-mark renders a
// dedicated dark-variant SVG instead of the light one on dark backgrounds.
function logoSvg(discFill: string): string {
